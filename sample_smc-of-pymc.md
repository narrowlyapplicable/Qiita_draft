...
1. 時刻 $t-1$における事後分布 $p(x_{T-1} | \{y_t\}_{t=1}^{T-1})$ をモンテカルロ近似したサンプル
   $$x_{T-1}^{(i)} \sim p(x_{T-1} | \{y_t\}_{t=1}^{T-1})$$
   がある。
...
4. 各サンプルを重みに従って再サンプリングする。
   すなわち、確率 $\frac{w_T^{(i)}}{\sum_i{w_T^{(i)}}}$で $x_T^{(i)}$をサンプリングし、所定回数くり返した結果を$p(x_T | \{y_t\}_{t=1}^T)$のモンテカルロ近似とする。
...
## 2.3. ベイズ推論としての解釈
本節では、上に記した粒子フィルタのアルゴリズムが、内部状態の分布 $p(x_t | \{y_t\}_{t=1}^T)$のベイズ推定になっていることを確認します。

モンテカルロ近似を行うには、
1. 対象空間上を満遍なくサンプリングし
2. 各サンプルが所望の分布に従っていなければならない。

ここでは事後分布 $p(x_T | \{Y_t\}_{t=1}^T)$ をサンプリングしたいので、サンプル $x_T^{(i)}$を得る確率は
...
そこで各サンプルの事後確率 $p(x_T = x_T^{(i)}| \{Y_t\}_{t=1}^T)$を求めて、確率の調整を行います。
...
これは単純に、$\{x_T^{(i)}\}_{i=1}^N$から確率$w_T^{(i)}$で$x_T^{(i)}$を引く……という**復元抽出**をくり返すだけです。
この復元抽出をN回くり返せば、得られたサンプルは事後分布$p(x_T | \{Y_t\}_{t=1}^T)$に従っており、事後分布をモンテカルロ近似することができます。
この過程を再サンプリングと呼びます。  
...
[図：正規分布から初めて混合正規へ遷移、混合正規内部での移動は困難]
...
- 初期状態
  - 逆温度の初期値を$\beta_0=0$とする。このとき$$p_{\beta=0}(\theta | X^{(n)}) = p(X^{(n)}|\theta)^0 p(\theta)=p(\theta) .$$   
    ...
    取得したサンプルを $\{\theta_0^{(i)}\}_{i=1}^N$ とし、これを更新していく。
1. 現状のサンプル$\{\theta_j^{(i)}\}_{i=1}^N$の有効サンプルサイズ（ESS）を計算し、それを元に$\beta$の増加幅$\Delta_j$を決める。 $$\beta_{j+1} := \beta_j + \Delta_j.$$ 
2. 増加した $\beta_{j+1}$ のもとで各サンプルの事後確率を計算し、再サンプリングの重み $\{w_j^{(i)}\}_{i=1}^N$ を計算する。
3. 重み$\{w_j^{(i)}\}_{i=1}^N$を付けた再サンプリングを実行し、逆温度 $\beta_{j+1}$ でのサンプル $\{\theta_{j+1}^{(i)}\}_{i=1}^N$ を得る。
...
- SMC Samplerでも、遷移する逆温度の差が大きすぎると、再サンプリング時に多くのサンプルが消滅することになります。これは非効率なだけでなく、サンプルが広い領域をカバーできなくなるため、モンテカルロ近似の精度確保の上で致命的です。
  - これは $p_{\beta=j}(\theta | X^{(n)})$と $p_{\beta=j+1}(\theta | X^{(n)})$の違いが大きくなると、前者から得たサンプルが後者に適合しなくなることによります。
